...
- **Database**: PostgreSQL (plan selectable during deployment - free tier available if unused)
- **File storage**: Uses `/tmp` (files are ephemeral and will be lost on restart)
- **Build time**: Limited build minutes per month

**Cost**: Depends on database plan selected during deployment. For persistent file storage, upgrade services to starter tier.
